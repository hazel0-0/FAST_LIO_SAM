...
class ImuProcess
{
 public:
  ...
  void set_single_antenna_mode(bool enable);
  void set_single_antenna_heading(double yaw_enu);
  void update_latest_gnss_pos(const V3D &pos_enu);
 ...
 private:
  ...
  bool   single_antenna_mode_ = false;
  bool   single_antenna_heading_ready_ = false;
  double single_antenna_yaw_enu_ = 0.0;
  bool   latest_gnss_pos_valid_ = false;
  V3D    latest_gnss_pos_enu_ = V3D::Zero();
...
void ImuProcess::set_single_antenna_mode(bool enable)
{
  single_antenna_mode_ = enable;
}

void ImuProcess::set_single_antenna_heading(double yaw_enu)
{
  single_antenna_yaw_enu_ = yaw_enu;
  single_antenna_heading_ready_ = true;
}

void ImuProcess::update_latest_gnss_pos(const V3D &pos_enu)
{
  latest_gnss_pos_enu_ = pos_enu;
  latest_gnss_pos_valid_ = true;
}
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  bool enu_heading_available = false;
  double yaw_imu = 0.0;

  if (enu_init_en_ && !single_antenna_mode_ && gps_euler_received_)
  {
    // ==== 双天线模式: 从GPS euler获取航向 ====
    ...
    yaw_imu = atan2(R_enu_imu(1, 0), R_enu_imu(0, 0));
    enu_heading_available = true;
  ...
  else if (enu_init_en_ && single_antenna_mode_ && single_antenna_heading_ready_)
  {
    // ==== 单天线模式: 从移动方向获取航向 ====
    yaw_imu = single_antenna_yaw_enu_;
    enu_heading_available = true;
    ROS_INFO("  (Single antenna heading: yaw_enu=%.2f deg)", yaw_imu * 180.0 / M_PI);
  }

  if (enu_init_en_ && enu_heading_available)
  {
    ...
    // 使用实时GNSS ENU位置，补偿杆臂得到IMU位置
    // P_imu_enu = P_gnss_enu - R_enu_imu * T_gnss_in_imu
    if (latest_gnss_pos_valid_) {
      init_state.pos = latest_gnss_pos_enu_ - R_init * gnss_T_wrt_IMU_;
      ROS_INFO("ENU Init using real-time GNSS pos: (%.4f, %.4f, %.4f)",
               latest_gnss_pos_enu_.x(), latest_gnss_pos_enu_.y(), latest_gnss_pos_enu_.z());
    } else {
      init_state.pos = -R_init * gnss_T_wrt_IMU_;
      ROS_WARN("ENU Init: no GNSS position available, using origin with lever arm only");
    }
...
void ImuProcess::Process(const MeasureGroup &meas,  esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init_)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT) //初始化imu数据量大于阈值
    {
      if (enu_init_en_ && !single_antenna_mode_ && !gps_euler_received_)
      {
        ROS_WARN_THROTTLE(1.0, "ENU init enabled (dual antenna), waiting for GPS euler data on /gps/euler ...");
        return;
      }
      if (enu_init_en_ && single_antenna_mode_ && !single_antenna_heading_ready_)
      {
        ROS_WARN_THROTTLE(1.0, "ENU init enabled (single antenna), waiting for heading from movement direction ...");
        return;
      }
